    ...
    <div className="fixed inset-0 z-[2500] flex items-center justify-center p-3 sm:p-5 bg-black/85 backdrop-blur-md animate-fadeIn font-sans">
      <div className="bg-slate-950 border border-slate-800 rounded-3xl w-full max-w-md shadow-2xl shadow-black/90 text-white overflow-hidden flex flex-col relative">
        ...
        <div className="p-6 space-y-4 overflow-y-auto max-h-[calc(85vh-10rem)] custom-scrollbar font-mono text-xs">
          ...
          {/* Quick Demo Account Button (For Sign In) */}
          {mode === 'SIGN_IN' && (
            <button
              type="button"
              onClick={handleUseDemoAccount}
              className="w-full py-2 px-3 rounded-xl bg-slate-900/90 hover:bg-slate-800 border border-emerald-500/30 hover:border-emerald-500/60 text-emerald-300 text-xs font-bold transition flex items-center justify-center gap-2 group shadow-sm"
            >
              <Zap className="w-3.5 h-3.5 text-emerald-400 group-hover:scale-110 transition" />
              <span>[ USE DEMO ACCOUNT ] (demo@roadside.in)</span>
            </button>
          )}
